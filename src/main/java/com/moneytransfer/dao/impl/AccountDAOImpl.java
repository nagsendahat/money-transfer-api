...
public class AccountDAOImpl implements AccountDAO {
	...
	private static Logger log = LogManager.getLogger(AccountDAOImpl.class);
	...
	public List<Account> getAllAccounts() throws MoneyTransferAPIException {
		...
		try {
			...
			while (rs.next()) {
				Account acc = new Account(rs.getLong("AccountId"), rs.getLong("UserId"),
			...
			throw new MoneyTransferAPIException("getAccountById(): Error reading account data", e);
		} finally {
	 ...
	 * Get account by id
	 */
	public Account getAccountById(long accountId) throws MoneyTransferAPIException {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		Account acc = null;
		try {
			...
			rs = stmt.executeQuery();
			if (rs.next()) {
				acc = new Account(rs.getLong("AccountId"), rs.getLong("UserId"), rs.getBigDecimal("Balance"),
	 ...
	 * Create account
	 */
	public long createAccount(Account account) throws MoneyTransferAPIException {
		Connection conn = null;
		PreparedStatement stmt = null;
		...
		try {
			...
			if (affectedRows == 0) {
				...
				throw new MoneyTransferAPIException("Account Cannot be created");
			}
			...
			throw new MoneyTransferAPIException("createAccount(): Error creating user account " + account, e);
		} finally {
	 ...
	 */
	public int deleteAccountById(long accountId) throws MoneyTransferAPIException {
		Connection conn = null;
		PreparedStatement stmt = null;
		try {
			conn = H2DAOFactory.getConnection();
			...
			throw new MoneyTransferAPIException("deleteAccountById(): Error deleting user account Id " + accountId, e);
		} finally {
	 ...
	 */
	public int updateAccountBalance(long accountId, BigDecimal deltaAmount) throws MoneyTransferAPIException {
		Connection conn = null;
		...
		try {
			...
			if (targetAccount == null) {
				throw new MoneyTransferAPIException("updateAccountBalance(): fail to lock account : " + accountId);
			}
			...
			if (balance.compareTo(MoneyUtil.zeroAmount) < 0) {
				throw new MoneyTransferAPIException("Not sufficient Fund for account");
			}
			...
			try {
				...
				throw new MoneyTransferAPIException("Fail to rollback transaction", re);
			}
		} finally {
			DbUtils.closeQuietly(conn);
	 ...
	 */
	public int transferAccountBalance(TransferRequest userTransaction) throws MoneyTransferAPIException {
		int result = -1;
		Connection conn = null;
		PreparedStatement lockStmt = null;
		PreparedStatement updateStmt = null;
		...
		try {
			...
			if (fromAccount == null || toAccount == null) {
				throw new MoneyTransferAPIException("Fail to lock both accounts for write");
			}
			...
			if (fromAccountLeftOver.compareTo(MoneyUtil.zeroAmount) < 0) {
				throw new MoneyTransferAPIException("Not enough Fund from source Account ");
			}
			// proceed with update
			updateStmt = conn.prepareStatement(SQL_UPDATE_ACC_BALANCE);
