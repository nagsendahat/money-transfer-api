...
public class TestAccountBalance {
	...
	private static Logger log = LogManager.getLogger(TestAccountDAO.class);
	...
	@Test
	public void testAccountSingleThreadSameCcyTransfer() throws MoneyTransferAPIException {
		...
		TransferRequest transaction = new TransferRequest("EUR", transferAmount, 3L, 4L);
	...
	@Test
	public void testAccountMultiThreadedTransfer() throws InterruptedException, MoneyTransferAPIException {
		final AccountDAO accountDAO = h2DaoFactory.getAccountDAO();
		...
		for (int i = 0; i < THREADS_COUNT; i++) {
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						TransferRequest transaction = new TransferRequest("USD",
	...
	@Test
	public void testTransferFailOnDBLock() throws MoneyTransferAPIException, SQLException {
		...
		try {
			...
			if (fromAccount == null) {
				throw new MoneyTransferAPIException("Locking error during test, SQL = " + SQL_LOCK_ACC);
			}
			...
			TransferRequest transaction = new TransferRequest("GBP", transferAmount, 6L, 5L);
